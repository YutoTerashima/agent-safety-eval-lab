...
Purpose: Trace-level policy simulation for allow/review/deny routing.
...
MODULE_PURPOSE = 'Trace-level policy simulation for allow/review/deny routing.'
...
def route_trace(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def summarize_policy_routes(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = route_trace(materialized)
...
def tool_risk_adjustment(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
