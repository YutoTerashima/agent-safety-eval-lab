...
"""Project-specific research helpers for Agent Safety Eval Lab.

Purpose: Threshold and calibration utilities for unsafe-score operating points.
...
MODULE_PURPOSE = 'Threshold and calibration utilities for unsafe-score operating points.'
...
def threshold_sweep(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def recommended_threshold(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = threshold_sweep(materialized)
...
def operating_points(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
